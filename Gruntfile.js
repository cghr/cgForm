...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    meta: {
      modules: 'angular.module("cgForm", [<%= srcModules %>]);',
      tplmodules: 'angular.module("cgForm.tpls", [<%= tplModules %>]);',
      all: 'angular.module("cgForm", ["cgForm.tpls", <%= srcModules %>]);',
      banner: ['/*',
               ' * <%= pkg.name %>',
               ' * <%= pkg.homepage %>\n',
  ...
  //Common cgForm module containing all modules for src and templates
  ...
  function findModule(name) {
    ...
    var module = {
      name: name,
      moduleName: enquote('cgForm.' + name),
  ...
  function dependenciesForModule(name) {
    ...
    .forEach(function(contents) {
      ...
      dependencies.split(',').forEach(function(dep) {
        if (dep.indexOf('cgForm.') > -1) {
          var depName = dep.trim().replace('cgForm.','').replace(/['"]/g,'');
